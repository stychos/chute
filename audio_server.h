...
void initializeWAVHeader(WAVHeader &header, uint32_t sampleRate, uint16_t bitsPerSample, uint16_t numChannels) {
  ...
  header.chunkSize = 0xFFFFFFFF; // Max value for streaming (unknown length)
  ...
  header.bitsPerSample = 16; // Always send 16-bit for browser compatibility
  header.byteRate = (sampleRate * 16 * numChannels) / 8;
  header.blockAlign = (16 * numChannels) / 8;
  header.subchunk2Size = 0xFFFFFFFF; // Max value for streaming (unknown length)
}
...
void mic_i2s_init() {
  ...
  i2s_config_t i2sConfig = {
    ...
    .channel_format = I2S_CHANNEL_FMT_ONLY_LEFT, // Mono audio (INMP441 L/R pin to GND)
    //CAMERA_MODEL_XIAO_ESP32S3
...
void handleAudioStream() {
  ...
  uint8_t i2sBuffer[bufferSize];
  int16_t outBuffer[bufferSize / 4]; // 1 x 16-bit sample per 32-bit I2S frame
  size_t bytesRead = 0;
  ...
  while (true) {
    ...
    // Read 32-bit I2S data from DMA
    i2s_read(I2S_PORT, i2sBuffer, bufferSize, &bytesRead, portMAX_DELAY);

    // Convert 32-bit samples to 16-bit (take upper 16 bits of each 32-bit sample)
    if (bytesRead > 0) {
      ...
      for (int i = 0; i < samplesRead; i++) {
        ...
        outBuffer[i] = (int16_t)amplified;
      }
      size_t outBytes = samplesRead * sizeof(int16_t);
      Audioclient.write((uint8_t *)outBuffer, outBytes);
      totalDataSize += outBytes;
    }
  }
}
